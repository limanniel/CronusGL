...
SceneNode_Static::SceneNode_Static(Model* model, glm::vec3 position, Rotation rotation, glm::vec3 scale)
	: SceneNode(model, position, rotation, scale)
{
	// Static Buffer Gen
	glGenBuffers(1, &_VertexBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, _VertexBuffer);
	glBufferData(GL_ARRAY_BUFFER, _Model->Mesh->indexed_Vertices.size() * sizeof(glm::vec3), &_Model->Mesh->indexed_Vertices[0], GL_STATIC_DRAW);

	glGenBuffers(1, &_UVBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, _UVBuffer);
	glBufferData(GL_ARRAY_BUFFER, _Model->Mesh->indexed_UVCoords.size() * sizeof(glm::vec2), &_Model->Mesh->indexed_UVCoords[0], GL_STATIC_DRAW);

	glGenBuffers(1, &_NormalBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, _NormalBuffer);
	glBufferData(GL_ARRAY_BUFFER, _Model->Mesh->indexed_Normals.size() * sizeof(glm::vec3), &_Model->Mesh->indexed_Normals[0], GL_STATIC_DRAW);
...
SceneNode_Static::~SceneNode_Static()
{
...
void SceneNode_Static::Update()
{
	SceneNode::Update();
}

void SceneNode_Static::Render()
{
